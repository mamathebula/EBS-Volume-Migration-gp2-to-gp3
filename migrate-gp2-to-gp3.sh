...
migrate_region() {
  ...
  echo "$VOLUMES" | while read -r vid size state instance; do
    ...
    ERROR=$(aws ec2 modify-volume --region "$region" --volume-id "$vid" --volume-type "$NEW_TYPE" --output text 2>&1 || true)
    if echo "$ERROR" | grep -qi "error\|failed\|exception"; then
      echo "  ✗ $vid failed"
      echo "    Reason: $ERROR"
      failed=$((failed + 1))
    else
    ...
    echo "$success" > /tmp/gp2_success
    echo "$failed" > /tmp/gp2_failed
  ...
  success=$(cat /tmp/gp2_success 2>/dev/null || echo 0)
  failed=$(cat /tmp/gp2_failed 2>/dev/null || echo 0)
  ...
  echo ""
  echo "Done with $region. Success: $success, Failed: $failed. Saving $(cents_to_dollars $SAVINGS_CENTS)/month."
}
